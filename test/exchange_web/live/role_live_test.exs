defmodule ExchangeWeb.RoleLiveTest do
  ...
  defp login_admin_user(%{conn: conn}) do
    {:ok, user} =
      Exchange.Accounts.register_user(%{email: "test@test.com", password: "Password123!"})

    {:ok, admin_user} = Exchange.Accounts.add_role_to_user(user, "Admin")

    conn =
      conn
      |> Map.replace!(:secret_key_base, ExchangeWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})

    %{
      conn: ExchangeWeb.ConnCase.log_in_user(conn, admin_user),
      role: Exchange.Accounts.get_role!(1)
    }
  end
  ...
  describe "Index" do
    ...
    test "deletes role in listing", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/roles")
      role = Exchange.Accounts.get_role_by_name!("User")
