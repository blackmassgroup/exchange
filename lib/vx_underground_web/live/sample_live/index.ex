defmodule VxUndergroundWeb.SampleLive.Index do
  ...
  defp kickoff_triage_report(upload, sample) do
    with(
      ...
      {:ok, id} <-
        VxUnderground.Services.Triage.upload(presigned_url) |> IO.inspect(label: :triage_upload),
      {:ok, hashes} <-
        VxUnderground.Services.Triage.get_sample(id) |> IO.inspect(label: :triage_get),
