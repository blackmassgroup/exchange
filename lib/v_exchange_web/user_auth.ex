defmodule VExchangeWeb.UserAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_v_exchange_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      VExchangeWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_user(conn, _opts) do
    ...
    assign(conn, :current_user, VExchange.Repo.Local.preload(user, :role))
  end
  ...
  the current_user:
      ...
      defmodule VExchangeWeb.PageLive do
        use VExchangeWeb, :live_view

        on_mount {VExchangeWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{VExchangeWeb.UserAuth, :ensure_authenticated}] do
    ...
    case session do
      %{"user_token" => user_token} ->
        Phoenix.Component.assign_new(socket, :current_user, fn ->
          ...
          |> VExchange.Repo.Local.preload(:role)
        end)
  ...
  defp signed_in_path(_conn), do: ~p"/samples"
end
